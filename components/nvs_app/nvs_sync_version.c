...
void nvs_sync_version(void *pvParameters) {
    while (1) {
        ...
        if (xSemaphoreTake(sync_trigger_sem, pdMS_TO_TICKS(SECURITY_TIMEOUT_MS)) == pdTRUE) {
            ...
            ESP_LOGI(TAG, "[NVS] Verificación periódica de versión.");
        }
        ...
        while (!node_mesh_info.is_synchronized) {
            ...
            // 1. ABRIR NVS (Usa el mismo namespace que en el guardado: "storage")
            esp_err_t err = nvs_open("storage", NVS_READONLY, &handle);
            if (err == ESP_OK) {
                // 2. LEER COMO U32 (Igual a como se guardó) con la llave "sync_ver"
            ...
            // 3. Preparar paquete de petición
            app_packet_t packet;
            ...
            // Nota: node_mesh_info.is_synchronized debe ser puesto en true 
            // cuando llegue el mensaje MQTT de respuesta exitosa.
        }
