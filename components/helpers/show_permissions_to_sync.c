...
void show_permissions_to_sync(const sync_data_event_t *sync_ev) {
  ESP_LOGI(TAG, "[HELPERS] === DATOS RECIBIDOS (V.%lu) ===", sync_ev->version);
  ...
  for (int i = 0; i < num_festivos; i++) {
    ESP_LOGI(TAG, "[HELPERS] 📅 Festivo: %02d/%02d/%d", lista_f[i].dia, lista_f[i].mes, lista_f[i].anio);
  }
  ...
  while (offset < sync_ev->permisos_len) {
    ...
    ESP_LOGI(TAG, "[HELPERS]💳 Tarjeta ID: %lu | Reglas: %d", tarjeta_id, num_reglas);
        
    offset += 5;
    
    for (int r = 0; r < num_reglas; r++) {
      regla_t *regla = (regla_t *)(p_ptr + offset);
      ESP_LOGI(TAG, "[HELPERS]   -> [%d] %02d:%02d:%02d a %02d:%02d:%02d", regla->dia_tipo, regla->h_i, regla->m_i, regla->s_i, regla->h_f, regla->m_f, regla->s_f);
