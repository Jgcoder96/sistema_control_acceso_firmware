...
void subscription_manager_sync_device(cJSON *root) {
  ...
  cJSON *curr_page = cJSON_GetObjectItemCaseSensitive(item, "current_page"); 
  cJSON *tot_pages = cJSON_GetObjectItemCaseSensitive(item, "total_pages");   
  ...
  if (!cJSON_IsNumber(version) || !cJSON_IsNumber(curr_page) || !cJSON_IsNumber(tot_pages) || !cJSON_IsString(mac_b64)) {
    ESP_LOGE(TAG, "[MQTT] JSON de Sync mal formado (faltan páginas o versión)");
    return;
  }
